...
export class AudioSynthesizer {
  constructor(ctx) {
    this.ctx = ctx;
    this.gunNoiseBuffer = null;
    this.activeGunSources = [];
  }
  ...
  playGunFire() {
    try {
      ...
      if (!this.gunNoiseBuffer) {
        const bufLen = Math.floor(audioCtx.sampleRate * 1.5);
        this.gunNoiseBuffer = audioCtx.createBuffer(1, bufLen, audioCtx.sampleRate);
        const data = this.gunNoiseBuffer.getChannelData(0);
        for (let i = 0; i < bufLen; i++) data[i] = Math.random() * 2 - 1;
      }
      
      for (const s of this.activeGunSources) {
        try { s.stop(); } catch {}
      }
      this.activeGunSources = [];
      ...
      boomSrc.buffer = this.gunNoiseBuffer;
      ...
      crackSrc.buffer = this.gunNoiseBuffer;
      ...
      // Ultra-low rumble
      const rumbleSrc = audioCtx.createBufferSource();
      rumbleSrc.buffer = this.gunNoiseBuffer;
      ...
      this.activeGunSources.push(boomSrc, crackSrc, rumbleSrc);
    } catch (e) {
