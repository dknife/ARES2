...
export class TrafficSubsystem {
  constructor(ctx, makeGLTFLoader) {
    ...
    this.TRAFFIC_OFF_COLOR = new ctx.THREE.Color(0x666666);
  }
  ...
  clearAllSlots() {
    for (let i = 0; i < this.trafficSlotState.length; i++) this.clearSlot(i);
  }
  ...
  setSlotOn(i, value) {
    ...
    for (const m of s.materials) {
      if (m.color    !== undefined) m.color.copy(s.on ? onCol : this.TRAFFIC_OFF_COLOR);
  ...
  placeLamps() {
    ...
    this.makeGLTFLoader().load(TRAFFIC.lamp, (gltf) => {
      ...
      for (let i = 0; i < this.trafficSlots.length; i++) {
        ...
        const light = this.makeSlotLight(this.trafficSlots[i], color); this.ctx.scene.add(light);
  ...
  placeHands() {
    ...
    for (let i = 0; i < n; i++) {
      ...
      this.makeGLTFLoader().load(url, (gltf) => {
        if (this.ctx.disposed || myToken !== this.trafficLoadToken) {
          gltf.scene.traverse((o) => {
            if (o.isMesh || o.isSprite) {
              o.geometry?.dispose?.();
        ...
        const light = this.makeSlotLight(slot, color); this.ctx.scene.add(light);
  ...
  setupTraffic(root) {
    ...
    const slotW = span / n;
    this.trafficSlots = [];
    for (let i = 0; i < n; i++) this.trafficSlots.push({ x: start + step * i, z: tcn.z, width: slotW });
    this.placeLamps();
  }
