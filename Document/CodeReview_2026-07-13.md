...
## 📌 다음 작업 — 피코 펌웨어 일괄 업데이트 (2026-07-14 예약)

**결정**: 펌웨어 수정·기기 업로드는 다음 작업 세션에 일괄 진행한다(기기 확보 시).
한 번의 업로드로 아래를 모두 반영한다:

1. **이미 커밋됨 — 기기 반영 대기**: `gun.py` fire_once = 65535 × **0.1s(100ms)**
   — 교육용 단순판 재작성(`8150dcf`) + Jihun 실측 튜닝 반영(`3b176c3`,
   0.33s 는 2발 격발 → 0.1s 로 50연발 무오작동 확인). 실기기 업로드 여부 확인.
2. **F1 수정(안전·최우선)**: 버퍼 오버플로우 시 완성 라인을 먼저 추출한 뒤
   미완성 꼬리만 절단 — 비상정지(STOP_ALL) 유실 차단 (`main.py:85,178`).
3. **F3 수정**: `SING`·`CALIB_START` 에 `_interruptible_sleep` 패턴 적용 —
   블로킹 중 비상정지 폴링.
4. **F5 수정(여유 시)**: 개행 없는 잔여 수신 조각에 수명(age) 부여.
5. **업로드·검증**: Thonny/ampy 로 `Pico/*.py` 업로드(9600 baud) 후
   ①시간형 이동 중 비상정지 ②SING 중 비상정지 ③발사 3연발(65535×330ms) 확인.
