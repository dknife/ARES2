...
\subsection{문제 유형과 대응 패턴}

\subsubsection{유형 A --- 거대 클로저에 갇힌 서브시스템 (\code{simulation.js})}
...
\subsubsection{유형 B --- 거대 디스패처 + 핸들러 더미}
...
\subsubsection{유형 C --- 데이터 덩어리 / 만능 함수}
...
\subsection{우선순위 로드맵 (위험 낮음 → 가치 높음)}
\begin{center}
\renewcommand{\arraystretch}{1.25}
