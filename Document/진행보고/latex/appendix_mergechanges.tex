...
%% ============================================================
\chaptersummary{중간 점검 통합 머지로 반영된 변경을 부록 A 기준과 대비해 정리한다. 웹·피코·시뮬 트랙별로 주요 코드 변경 사항을 요약한다.}
\chapter{중간 점검 머지 --- 주요 코드 변경 (부록 A 대비)}
\label{app:merge}

본 부록은 \textbf{5일차 작업 브랜치 통합 머지}(2장 5일차 참조)로 \code{main}에 반영된
주요 코드 변경을, \textbf{\ref{app:code}(1일차 코드 분석 종합 보고서)가 기록한 머지 전
코드 상태}와 대조해 정리한 \emph{중간 점검}이다. 부록 A가 \emph{통합 전} 코드를 기술한
기준선이라면, 본 부록은 그 위에 1$\cdot$2차 Refactoring 계획(\ref{app:refactor1}$\cdot$%
...
\caption{핵심 파일 전후 비교 --- \ref{app:code} 기준선 대비 통합 후 \code{main}}
...
\section{시뮬레이터 --- 모놀리스 \code{simulation.js}의 모듈화 (유형 A 실행)}
...
\captionof{listing}{실제 코드 --- \code{Sim\_Parts/context.js}: \code{SimContext}가 공유 ctx(=this)로 서브시스템을 조립}
...
\captionof{listing}{실제 코드 --- \code{simulation.js}: 거대 클로저 대신 \code{buildSim} 모듈을 조립만 하는 얇은 오케스트레이터(공개 API \code{setupSimulation} 유지)}
\begin{minted}{javascript}
...
\section{피코 펌웨어 --- 설정 체계와 명령 프로토콜 변경}
...
\captionof{listing}{실제 파일 --- \code{Pico/ModelFactorySetting.txt}(발췌): JSON 대신 주석 달린 텍스트 설정}
\begin{minted}{ini}
...
\captionof{listing}{실제 코드 --- \code{system\_config.py}: 텍스트(키=값/키:값) 파서. 파일이 없으면 설명 템플릿을 자동 생성}
\begin{minted}{python}
...
\captionof{listing}{실제 코드 --- \code{system\_config.py}: 저장 시 기존 주석$\cdot$가이드라인을 보존하고 값만 교체}
\begin{minted}{python}
...
\begin{table}[htbp]\centering\small
\caption{명령 프로토콜 변경 --- \ref{app:code} 명령 목록 대비 신규$\cdot$확장}
\renewcommand{\arraystretch}{1.2}
...
\captionof{listing}{실제 코드 --- \code{process\_data.py}: 속도 인자를 0--100\%(서보는 0.0--1.0)로 클램프 (5일차 모터 속도 제한)}
\begin{minted}{python}
...
\section{웹 인터페이스 --- 라우팅 정리와 블록코딩$\cdot$BLE 연동}
...
\captionof{listing}{실제 코드 --- \code{ui.js}: \code{main.js}에서 분리한 UI 보조 함수(상태별 버튼 라벨)}
\begin{minted}{javascript}
// ui.js — UI 전용 보조 함수들
...
\captionof{listing}{실제 코드 --- \code{commandexecutor.js}: 블록 → 명령 문자열에 속도 인자 추가}
\begin{minted}{javascript}
...
\section{진입점$\cdot$자산 --- 페이지 구성과 3D 모델 경량화}
...
\small
1$\cdot$2차 Refactoring 계획(\ref{app:refactor1}$\cdot$\ref{app:refactor2})의 핵심
