...
%% ============================================================
\section{중간 보고 시점 코드 분석 (부록 A 구조 재적용)}
\label{app:midcode}

본 부록은 \textbf{부록~\ref{app:code}(1일차 「코드 분석 종합 보고서」)의 장 구성을 중간 보고
시점의 실제 코드에 다시 적용한 재분석}이다. 부록~\ref{app:merge}(부록 D)가 통합 머지로 반영된
...
\subsection{사용 하드웨어와 연결 핀 (부록 A 3장 재적용)}
...
\begin{lstlisting}[style=py, caption={\code{Pico/pins.py}(발췌) --- 부록 A와 동일한 핀 배치(중앙 집중)}]
...
\subsection{보드 펌웨어 (Pico) --- 명령 처리 · 설정 · 안전 (부록 A 4장 재적용)}
...
\begin{lstlisting}[style=py, caption={\code{process\_data.py} --- 접두어 매칭 디스패처(유형 B, 부록 A 유지)}]
...
\begin{lstlisting}[style=py, caption={\code{process\_data.py} --- 중단 가능 sleep(50ms 슬라이스 + 60초 상한)}]
...
\begin{lstlisting}[style=py, caption={\code{main.py} --- \code{\_poll\_abort()}: 동작 중 STOP 선별 소비 + bytes 버퍼(한글 유실 수정)}]
...
\begin{lstlisting}[style=py, caption={\code{system\_config.py} --- 텍스트(키=값/키:값) 파서와 부팅 안전화}]
...
\subsection{서비스 계층 --- 웹 애플리케이션 구조 (부록 A 5장 재적용)}
...
\begin{lstlisting}[style=js, caption={\code{main.js} --- 엔트리가 import하는 모듈(부록 A 의존 그래프에 \code{ui.js} 추가)}]
...
\begin{lstlisting}[style=js, caption={\code{ui.js} --- \code{main.js}에서 분리한 상태별 버튼 라벨 헬퍼}]
...
\subsection{블루투스 지원 --- 응답 동기화 (부록 A 6장 재적용)}
...
\begin{lstlisting}[style=js, caption={\code{bluetooth.js} --- 동적 타임아웃(BATCH는 하위 명령 합산)과 기대 응답 접두어 검증}]
...
\begin{lstlisting}[style=js, caption={\code{bluetooth.js} --- 비상정지: 대기 중 응답 promise를 즉시 취소해 전송 큐를 비운다}]
...
\subsection{코드 블록 구현 --- 블록 정의와 실행기 (부록 A 7장 재적용)}
...
\begin{lstlisting}[style=js, caption={\code{commandexecutor.js} --- else-if(\code{IF0..IFn}) 순회와 속도 인자 명령}]
...
\begin{lstlisting}[style=js, caption={\code{blocklyconfig.js} --- 모델별 동적 라벨 설정(\code{GET\_NAMES} 수신값으로 탭 한글 이름 갱신)}]
...
\subsection{WebGL 3D 시뮬레이션 --- 모듈화된 서브시스템 (부록 A 8장 재적용)}
...
\begin{lstlisting}[style=js, caption={\code{Sim\_Parts/context.js} --- \code{SimContext}가 공유 ctx(=this)로 서브시스템을 조립}]
...
\begin{lstlisting}[style=js, caption={\code{Sim\_Parts/render.js} --- 프레임 독립 애니메이션(초당 값 $\times$ dt)}]
...
\subsection{코드별 연계 구조 (부록 A 9장 재적용)}
...
\begin{lstlisting}[style=js, caption={\code{commandexecutor.js} --- 하드웨어/시뮬 이중 경로를 sink 하나로 추상화}]
...
\subsection{소스 코드별 API Reference --- 델타 (부록 A A.1$\sim$A.3 재적용)}
