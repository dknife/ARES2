...
시뮬레이터 사용법 & 씬 생성 도구(컴포넌트 9종$\cdot$개발자/사용자 모드) 사용 안내 & 부록 G \\
\bottomrule
\end{tabularx}
\end{center}
...
12일차에 구축한 \textbf{블록코딩 연계 씬 생성 도구}의 사용 안내서다. 사용자는 씬
드롭다운에서 저장된 씬을 골라 \textbf{모의실행/실험중단}으로 블록 코드를 장치 없이
실행하고, 개발자는 \textbf{Ctrl+E}로 진입해 우클릭 스폰(기본 도형$\cdot$OLED$\cdot$GLB 16종
목록), 기즈모$\cdot$Hierarchy(길게 클릭 이름 변경)$\cdot$인스펙터(트랜스폼$\cdot$컴포넌트
필드 편집)로 씬을 구성한 뒤 JSON 으로 저장한다. \textbf{컴포넌트 9종 레퍼런스}
(LED$\cdot$Buzzer$\cdot$Oled$\cdot$DC$\cdot$Servo$\cdot$UltraSonic$\cdot$Magnet$\cdot$Metal$\cdot$Gun)와
좌표계 규약(회전=부모$\cdot$이동=로컬$\cdot$오프셋 m$\cdot$스케일 비전파), 씬 배포 절차
(\code{Web/scenes/} + \code{manifest.json} 등록 $\to$ 사용자 드롭다운 노출)를 담았다.
