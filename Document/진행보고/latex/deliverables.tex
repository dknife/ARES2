...
각 산출물의 \textbf{전문은 참고문헌 뒤 부록}(부록 A$\sim$E)에 수록한다. 앞의 세 산출물
(부록 A$\sim$C)이 \emph{기준선 분석과 개선 설계}라면, 뒤의 두 산출물(부록 D$\cdot$E)은
그 설계가 통합 머지로 실행된 뒤의 \emph{중간 점검$\cdot$재분석}이다.
